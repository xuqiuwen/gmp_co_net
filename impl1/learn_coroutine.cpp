...
struct coro_ret {
  ...
  std::coroutine_handle<promise_type> coro_handle_;  // 协程句柄
  coro_ret(handle_type h) : coro_handle_(h) {}       // 构造函数
  ...
  bool move_next() {  // 恢复协程，返回是否结束
    coro_handle_.resume();
    return coro_handle_.done();
  }
  T get() {  // 通过promise获取数据，返回值
    return coro_handle_.promise().return_data_;
  }
  // promise_type就是承诺对象，承诺对象用于协程内外交流
  struct promise_type {
    promise_type() { std::cout << __func__ << std::endl; };
    ~promise_type() = default;
    auto get_return_object() {  // 生成协程返回值
...
int main(int argc, char* argv[]) {
  ...
  auto c_r = coroutine_7in7out();  // 这里就是初始化协程
